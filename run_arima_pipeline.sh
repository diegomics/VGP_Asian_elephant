...
INDEX2_JOB=$(sbatch --mail-user=${USER_MAIL} --partition=${PARTITION} --qos=${QUEUE} --output=${OUT_DIR}/std_logs/%x.%j.out --error=${OUT_DIR}/std_logs/%x.%j.err slurm/arima.index2.job)
INDEX2_JOB_ID=$(echo $INDEX2_JOB | cut -d ' ' -f4)


# === STEP 3/12: Map forward reads ===================================

MAP1_JOB=$(sbatch --dependency=afterok:${INDEX1_JOB_ID}:${INDEX2_JOB_ID} --mail-user=${USER_MAIL} --partition=${PARTITION} --qos=${QUEUE} --output=${OUT_DIR}/std_logs/%x.%j.out --error=${OUT_DIR}/std_logs/%x.%j.err slurm/arima.map1.job)
MAP1_JOB_ID=$(echo $MAP1_JOB | cut -d ' ' -f4)


# === STEP 4/12: Map reverse reads ===================================

MAP2_JOB=$(sbatch --dependency=afterok:${INDEX1_JOB_ID}:${INDEX2_JOB_ID} --mail-user=${USER_MAIL} --partition=${PARTITION} --qos=${QUEUE} --output=${OUT_DIR}/std_logs/%x.%j.out --error=${OUT_DIR}/std_logs/%x.%j.err slurm/arima.map2.job)
...
INDEX_JOB=$(sbatch --dependency=afterok:${DEDUP_JOB_ID} --mail-user=${USER_MAIL} --partition=${PARTITION} --qos=${QUEUE} --output=${OUT_DIR}/std_logs/%x.%j.out --error=${OUT_DIR}/std_logs/%x.%j.err slurm/arima.index.job)
INDEX_JOB_ID=$(echo $INDEX_JOB | cut -d ' ' -f4)


# === STEP 11/12: Make stats =========================================

STATS_JOB=$(sbatch --dependency=afterok:${INDEX_JOB_ID} --mail-user=${USER_MAIL} --partition=${PARTITION} --qos=${QUEUE} --output=${OUT_DIR}/std_logs/%x.%j.out --error=${OUT_DIR}/std_logs/%x.%j.err slurm/arima.stats.job)
STATS_JOB_ID=$(echo $STATS_JOB | cut -d ' ' -f4)


# === STEP 12/12: Remove intermediate files ==========================

#REMOVE_JOB=$(sbatch --dependency=afterok:${STATS_JOB_ID} --mail-user=${USER_MAIL} --partition=${PARTITION} --qos=${QUEUE} --output=${OUT_DIR}/std_logs/%x.%j.out --error=${OUT_DIR}/std_logs/%x.%j.err slurm/arima.remove.job)
